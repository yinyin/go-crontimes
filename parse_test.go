...
func TestParseElementText01Empty(t *testing.T) {
	r, err := parseElementText("", 0, 60, 60)
	if nil != err {
...
func TestParseElementText01UnexpectChar(t *testing.T) {
	if _, err := parseElementText("a", 0, 60, 60); nil == err {
	...
	if _, err := parseElementText("1,a", 0, 60, 60); nil == err {
		t.Error("expect error")
	} else if e, ok := err.(*ErrCronExpression); !ok {
		t.Errorf("unexpected error type: %#v", err)
	...
	if _, err := parseElementText("1-a/2", 0, 60, 60); nil == err {
		t.Error("expect error")
	} else if e, ok := err.(*ErrCronExpression); !ok {
		t.Errorf("unexpected error type: %#v", err)
	} else if (e.ErrType != ErrSyntaxUnexpectCharacter) || (e.CharIndex != 2) {
...
func TestParseElementText01OutOfRange(t *testing.T) {
	if _, err := parseElementText("0", 1, 12, 12); nil == err {
		t.Error("expect error")
	} else if e, ok := err.(*ErrCronExpression); !ok {
		t.Errorf("unexpected error type: %#v", err)
	...
	if _, err := parseElementText("13", 1, 12, 12); nil == err {
		t.Error("expect error")
	} else if e, ok := err.(*ErrCronExpression); !ok {
		t.Errorf("unexpected error type: %#v", err)
	...
	if _, err := parseElementText("3,13,5", 1, 12, 12); nil == err {
		t.Error("expect error")
	} else if e, ok := err.(*ErrCronExpression); !ok {
		t.Errorf("unexpected error type: %#v", err)
...
func TestParseElementText02R0059(t *testing.T) {
	if r, err := parseElementText("5", 0, 59, 59); nil != err {
		t.Errorf("unexpected error: %v", err)
	} else if !checkParsedElement(r, 1<<5, 0, 0) {
		t.Errorf("unexpected result: %#v", r)
	}
	if r, err := parseElementText("10-15", 0, 59, 59); nil != err {
		t.Errorf("unexpected error: %v", err)
	} else if !checkParsedElement(r, 0x3f<<10, 0, 0) {
		t.Errorf("unexpected result: %#v", r)
	}
	if r, err := parseElementText("10-15,1", 0, 59, 59); nil != err {
		t.Errorf("unexpected error: %v", err)
	} else if !checkParsedElement(r, 0x3F<<10|1<<1, 0, 0) {
		t.Errorf("unexpected result: %#v", r)
	}
	if r, err := parseElementText("10-15,1,50-59/3", 0, 59, 59); nil != err {
		t.Errorf("unexpected error: %v", err)
...
func TestParseElementText03R0006(t *testing.T) {
	if r, err := parseElementText("5", 0, 6, 7); nil != err {
		t.Errorf("unexpected error: %v", err)
	} else if !checkParsedElement(r, 1<<5, 0, 0) {
		t.Errorf("unexpected result: %#v", r)
	}
	if r, err := parseElementText("6-7", 0, 6, 7); nil != err {
		t.Errorf("unexpected error: %v", err)
	} else if !checkParsedElement(r, 0x3<<6, 0, 0) {
		t.Errorf("unexpected result: %#v", r)
	}
	if r, err := parseElementText("7", 0, 6, 7); nil != err {
		t.Errorf("unexpected error: %v", err)
	} else if !checkParsedElement(r, 1<<7, 0, 0) {
		t.Errorf("unexpected result: %#v", r)
	}
	if r, err := parseElementText("1,3L", 0, 6, 7); nil != err {
		t.Errorf("unexpected error: %v", err)
	} else if !checkParsedElement(r, 1<<1, 1<<3, 0) {
		t.Errorf("unexpected result: %#v", r)
	}
	if r, err := parseElementText("*", 0, 6, 7); nil != err {
		t.Errorf("unexpected error: %v", err)
